 ...
 */
	...
	if ($_SESSION['user']['droits']<3){
		...
		if ($nb_email==0) {
			...
			$form->ajoute_interrupteur('valider', 'bouton', 'Confirmer l\'envoi', 'bouton', 1, 'mailing/env_news');
			foreach ($form->entrees as $nom => $value) {
				$html.=$value['html'];
			}
			$html.="<p>La newsletter va être envoyé à $nb_email contact(s).</p>";

...
class Categories {
	...
	public function casquettes_arbre($id)
		...
		foreach($enfants as $id_enfant) {
			...
			if(DEBUG_LOG) error_log(date('d/m/Y H:i:s')." - total catégorie $id Enfant $id_enfant : ".implode(', ',$cas_enfant)."\n", 3, "tmp/debug.log");
			$casquettes=array_merge($casquettes,$cas_enfant);
		}
		$casquettes=array_unique($casquettes);
		if(DEBUG_LOG) error_log(date('d/m/Y H:i:s')." - total catégorie $id : ".implode(', ',$casquettes)."\n", 3, "tmp/debug.log");
		return $casquettes;	
	}	
